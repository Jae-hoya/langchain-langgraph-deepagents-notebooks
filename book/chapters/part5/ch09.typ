...
에이전트는 _상태를 유지하는 장기 실행 프로세스_이므로 일반 웹앱 호스팅과 다른 접근이 필요합니다. 체크포인터를 통한 상태 영속화, 백그라운드에서의 장시간 실행, WebSocket 기반 스트리밍 등은 전통적인 요청-응답 모델의 웹 호스팅에서는 지원이 어렵습니다. LangGraph Platform은 이러한 에이전트 특유의 요구사항을 처음부터 고려하여 설계된 배포 인프라입니다.
